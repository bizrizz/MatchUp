...
//    private init() { } // Prevents external instantiation
    ...
    let baseURL = "https://matchup-api.xyz/api/v1/users"
    ...
    // Function to fetch user locations and store them in UserDefaults
       func fetchUserLocations(userId: Int, completion: @escaping (Bool, Error?) -> Void) {
           guard let url = URL(string: "https://matchup-api.xyz/api/user-locations/user/\(userId)") else {
    ...
    func getAllUsers(completion: @escaping ([User]?, Error?) -> Void) {
        guard let url = URL(string: "https://matchup-api.xyz/api/v1/users") else { completion(nil, nil); return }
    ...
    func getUserByEmail(_ email: String, completion: @escaping (User?, Error?) -> Void) {
        guard let url = URL(string: "https://matchup-api.xyz/api/v1/users/\(email)") else { completion(nil, nil); return }
        URLSession.shared.dataTask(with: url) { data, _, error in
            if let data = data {
    ...
    func registerUser(_ user: User, completion: @escaping (Bool, Error?) -> Void) {
        guard let url = URL(string: "https://matchup-api.xyz/api/v1/users") else { completion(false, nil); return }
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        request.addValue("application/json", forHTTPHeaderField: "Content-Type")
    ...
    func deleteUser(userId: Int, completion: @escaping (Bool, Error?) -> Void) {
        guard let url = URL(string: "https://matchup-api.xyz/api/v1/users/\(userId)") else { completion(false, nil); return }
        var request = URLRequest(url: url)
    ...
    func updateUser(userId: Int, userName: String, userEmail: String, userNickName: String, userPassword: String, completion: @escaping (Bool, Error?) -> Void) {
        guard let url = URL(string: "https://matchup-api.xyz/api/v1/users/\(userId)") else { completion(false, nil); return }
        var request = URLRequest(url: url)
        request.httpMethod = "PUT"
        request.addValue("application/json", forHTTPHeaderField: "Content-Type")
        let body: [String: Any] = ["userName": userName, "userEmail": userEmail, "userNickName": userNickName, "userPassword": userPassword]
        request.httpBody = try? JSONSerialization.data(withJSONObject: body)
        URLSession.shared.dataTask(with: request) { _, _, error in
            completion(error == nil, error)
        }.resume()
    ...
    // Location Endpoints
    func getAllLocations(completion: @escaping ([Location]?, Error?) -> Void) {
        guard let url = URL(string: "https://matchup-api.xyz/api/v1/locations") else { completion(nil, nil); return }
        URLSession.shared.dataTask(with: url) { data, _, error in
            if let data = data {
    ...
    func registerLocation(_ location: Location, completion: @escaping (Bool, Error?) -> Void) {
        guard let url = URL(string: "https://matchup-api.xyz/api/v1/locations") else { completion(false, nil); return }
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        request.addValue("application/json", forHTTPHeaderField: "Content-Type")
    ...
    func deleteLocation(locationId: Int, completion: @escaping (Bool, Error?) -> Void) {
        guard let url = URL(string: "https://matchup-api.xyz/api/v1/locations/\(locationId)") else { completion(false, nil); return }
        var request = URLRequest(url: url)
        request.httpMethod = "DELETE"
        URLSession.shared.dataTask(with: request) { _, _, error in
            completion(error == nil, error)
    ...
    func updateLocation(locationId: Int, locationActivePlayers: Int, completion: @escaping (Bool, Error?) -> Void) {
        guard let url = URL(string: "https://matchup-api.xyz/api/v1/locations/\(locationId)") else { completion(false, nil); return }
        var request = URLRequest(url: url)
        request.httpMethod = "PUT"
        request.addValue("application/json", forHTTPHeaderField: "Content-Type")
        let body: [String: Any] = ["locationActivePlayers": locationActivePlayers]
        request.httpBody = try? JSONSerialization.data(withJSONObject: body)
        URLSession.shared.dataTask(with: request) { _, _, error in
            completion(error == nil, error)
        }.resume()
    ...
    // Message Endpoints
    func getMessages(locationId: Int, completion: @escaping ([ChatMessage]?, Error?) -> Void) {
        guard let url = URL(string: "https://matchup-api.xyz/api/messages/\(locationId)") else { completion(nil, nil); return }
        URLSession.shared.dataTask(with: url) { data, _, error in
            if let data = data {
    ...
    func getUserLocations(userId: Int, completion: @escaping ([UserLocation]?, Error?) -> Void) {
        guard let url = URL(string: "https://matchup-api.xyz/api/user-locations/user/\(userId)") else { completion(nil, nil); return }
        URLSession.shared.dataTask(with: url) { data, _, error in
            if let data = data {
                let locations = try? JSONDecoder().decode([UserLocation].self, from: data)
                completion(locations, error)
    ...
    func addUserToLocation(userId: Int, locationId: Int, completion: @escaping (Bool, Error?) -> Void) {
        guard let url = URL(string: "https://matchup-api.xyz/api/user-locations") else { completion(false, nil); return }
        var request = URLRequest(url: url)
        request.httpMethod = "POST"
        request.addValue("application/json", forHTTPHeaderField: "Content-Type")
        let body: [String: Any] = ["userId": userId, "locationId": locationId]
        request.httpBody = try? JSONSerialization.data(withJSONObject: body)
        URLSession.shared.dataTask(with: request) { _, _, error in
            completion(error == nil, error)
        }.resume()
    }
    func removeUserFromLocation(userId: Int, locationId: Int, completion: @escaping (Bool, Error?) -> Void) {
        guard let url = URL(string: "https://matchup-api.xyz/api/user-locations/user/\(userId)/location/\(locationId)") else { completion(false, nil); return }
        var request = URLRequest(url: url)
        request.httpMethod = "DELETE"
        URLSession.shared.dataTask(with: request) { _, _, error in
            completion(error == nil, error)
